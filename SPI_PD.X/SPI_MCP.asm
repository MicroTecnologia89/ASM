...
;*********************************************************************************    
    ...
    CMD_WRITE_MCP   EQU	B'00010001'	; COMANDO ESCRITURA POT 0
	
	CLRF	POT_VALUE		; VARIABLE DE VALOR POTENCIOMETRO DIGITAL
   
	BSF	CS

PRINCIPAL

	MOVF	POT_VALUE,W		; MUEVO EL VALOR A W
	CALL	WRITE_MCP		; ESCRIBO POTENCIOMETRO
	CALL	RET_100ms			; RETARDO VISUALIZACION
	INCF	POT_VALUE,F		; INCREMENTO EN 1 VALOR DE POTENCIOMETRO
	GOTO	PRINCIPAL		; LOOP PRINCIPAL
	
WRITE_MCP
	MOVWF	AUX_MCP			; GUARDO EL VALOR A ESCRIBIR
	BCF	CS			; ACTIVO CHIP SELECT
	MOVLW	CMD_WRITE_MCP		; MOUEVO EL COMANDO A W
	CALL	SPI_DATA		; ESCRIBO EN SPI
	MOVF	AUX_MCP,W		; MUEVO EL VALOR A W REG
	CALL	SPI_DATA		; ESCRIBO EN SPI	
	BSF	CS			; REGRESA CHIP SELECT A ESTADO INACTIVO
	RETURN
;*********************************************************************************
;*********************************************************************************	
SPI_DATA
