...
;*********************************************************************************
	...
	MOVLW	0X27			; VALOR PARA 5kHz
	MOVWF	PR2			; CON PRESCALER DE 16
	
	MOVLW	B'00001100'		; CONFIGURO MODULO PWM
	MOVWF	CCP1CON			; ASIGNANDO FUNCIÓN
	
	CLRF	CCPR1L			; EN CERO EL DUTY
	
	MOVLW	B'00000111'		; CONFIGURO TIMER 2
	MOVWF	T2CON			; CON PREESCALER 16
...
#DEFINE	    INC_PWM	PORTA,0		; PIN DE INCREMENTO DE DUTY CLICLE
#DEFINE	    DEC_PWM	PORTA,1		; PIN DE DECREMENTO DE DUTY CICLE
	
	CLRF	DUTY
PRINCIPAL
	MOVFF	DUTY,CCPR1L		; CARGAR DUTY AL REGISTRO
	MOVFF	DUTY,LATB
	BTFSS	INC_PWM			; TESTEO EL BOTON DE INCREMENTO
	GOTO	SUMA_DUTY		; RUTINA DE INCREMENTO
	BTFSS	DEC_PWM			; TESTEO BOTON DE DECREMENTO
	GOTO	RESTA_DUTY		; RUTINA DE DECREMENTO
	GOTO	PRINCIPAL		; LOOP PRINCIPAL

SUMA_DUTY
;	BTFSS	INC_PWM			; TESTEO NUEVAMENTE EL PIN DE INCREMENTO
;	GOTO	SUMA_DUTY		; LOOP DE SEGURIDAD
	CALL	RET_10ms
	INCF	DUTY,F			; INCREMENTO EN 1 EL DUTY
	MOVF	DUTY,W			; MOEVO EL REGISTRO A W
	SUBLW	0X00			; RESTO 0X00
	BTFSS	STATUS,Z		; PREGUNTO POR EL BIT ZERO
	GOTO	PRINCIPAL		; REGRESO AL MAIN LOOP
	MOVLW	0XFF			; RECARGO CON VALOR MAXIMO
	MOVWF	DUTY			; EL REGISTRO
	GOTO	PRINCIPAL		; REGRESO AL MAIN LOOP	
RESTA_DUTY
	CALL	RET_10ms	
	DECF	DUTY,F			; INCREMENTO EN 1 EL DUTY
	MOVF	DUTY,W			; MOEVO EL REGISTRO A W
	SUBLW	0XFF			; RESTO 0X00
	BTFSS	STATUS,Z		; PREGUNTO POR EL BIT ZERO
	GOTO	PRINCIPAL		; REGRESO AL MAIN LOOP
	MOVLW	0X00			; RECARGO CON VALOR MAXIMO
	MOVWF	DUTY			; EL REGISTRO
	GOTO	PRINCIPAL		; REGRESO AL MAIN LOOP		
